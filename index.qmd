...
penguins |>
  ggplot(aes(x = bill_length_mm, y = bill_depth_mm, color = species)) +
  geom_smooth(aes(lty = island), method = "lm", se = FALSE) +
  geom_smooth(aes(group = island), method = "lm", se = FALSE, color = "black") +
...
penguins |>
  ...
  geom_histogram(aes(y = stat(density)), alpha=0.4, position = "identity") +
  theme(text = element_text(size = 20))
```
...
- by Hadley Wickhame [@wickhamTidyData2014]
...
But: Objects are just implementation details
